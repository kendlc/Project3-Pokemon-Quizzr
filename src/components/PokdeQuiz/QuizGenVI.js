...
const QuizGenVI = () => {
    ...
    useEffect( () => {
        ...
        const getPokeGuess = () => {
            for (let i = 0; i < 5; i++){
                axios.get(`https://pokeapi.co/api/v2/pokemon/${(Math.floor(Math.random() * (721 - 650 + 1)) + 650)}`)
                .then( async ({data}) => {
                    const questionText = data.name
                    const id = data.id
                    const image = data.sprites.other["official-artwork"]["front_default"]
                     await axios.get(`https://pokeapi.co/api/v2/pokemon?offset=650&limit=721`)
                    .then( async ({data}) => {
